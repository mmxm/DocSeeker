...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function _executeLoadDocumentInViewer(docId, docTitle, targetPage, occurrences, targetRect = null, targetYRatio = 0, targetOccId = null, targetScrollTop = null, targetOccIndex = null) {
    ...
    const updateCacheUI = (status, progress, downloadedBytes = 0, totalBytes = 0) => {
      ...
      if (isComplete) {
        ...
        if (mbDl && mbTot && progress > 0) {
          ...
          viewerCacheBadge.title = `Mise en cache hors-ligne : ${mbDl} Mo sur ${mbTot} Mo (${progress}%) - Cliquez pour suspendre`;
        } else if (progress > 0) {
    ...
    currentViewerUpdateCacheUI = updateCacheUI;
    ...
    if (isSameDoc && pdfFrame.contentWindow && pdfFrame.contentWindow.PDFViewerApplication) {
      ...
      (async () => {
        ...
        let win = null;
        try {
          win = pdfFrame?.contentWindow;
          if (win && win.location && win.location.href && win.PDFViewerApplication && win.PDFViewerApplication.initialized && typeof win.PDFViewerApplication.open === "function") {
