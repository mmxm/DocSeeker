...
def index_pdf_file(file_path: str, original_filename: str, custom_title: Optional[str] = None) -> Dict[str, Any]:
    ...
    1. Extrait les métadonnées et le nombre de pages.
    2. Génère une miniature de couverture (première page).
    3. Extrait pour chaque page le texte et la liste géométrique des mots (bounding boxes).
    4. Insère dans SQLite et FTS5.
    """
    doc = pymupdf.open(file_path)
    total_pages = len(doc)
    file_size = os.path.getsize(file_path)

    # Titre : soit custom, soit métadonnée PDF, soit nom du fichier nettoyé
    pdf_meta_title = doc.metadata.get("title", "").strip() if doc.metadata else ""
    title = custom_title or pdf_meta_title or os.path.splitext(original_filename)[0]
    ...
    if existing:
        doc_id = existing["id"]
        # Supprimer anciennes données liées
        cursor.execute("DELETE FROM pages WHERE doc_id = ?", (doc_id,))
        cursor.execute("DELETE FROM pages_fts WHERE doc_id = ?", (doc_id,))
        cursor.execute("""
            UPDATE documents 
            SET title = ?, total_pages = ?, file_size = ?, created_at = CURRENT_TIMESTAMP 
            WHERE id = ?
        """, (title, total_pages, file_size, doc_id))
    else:
        cursor.execute("""
            INSERT INTO documents (filename, title, total_pages, file_size) 
            VALUES (?, ?, ?, ?)
        """, (original_filename, title, total_pages, file_size))
